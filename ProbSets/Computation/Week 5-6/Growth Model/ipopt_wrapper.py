...
def EV_F(X, k_init, n_agents, theta):
...
def V_INFINITY(k=[], theta):
    ...
    for i in range(len(theta)):
        c=theta[i]*output_f(k,e, theat)
        v_vec += utility(c,e)/(1-beta)
    v_infinity = v_vec / len(theta)
    return v_infinity
...
def EV_GRAD_F(X, k_init, n_agents, theta):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F(xAdj, k_init, n_agents, theta)

            xAdj[ixN]=X[ixN] - h
            fx1=EV_F(xAdj, k_init, n_agents, theta)
...
def EV_GRAD_F_ITER(X, k_init, n_agents, grid, theta):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F_ITER(xAdj, k_init, n_agents, grid, theta)

            xAdj[ixN]=X[ixN] - h
            fx1=EV_F_ITER(xAdj, k_init, n_agents, grid, theta)
...
def EV_G(X, k_init, n_agents, theta):
    ...
    f_prod=output_f(k_init, lab, theta)
